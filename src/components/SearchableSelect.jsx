...
export function SearchableSelect({ items, selectedItem, onSelect, placeholder = "Seleccionar...", label = "Todos" }) {
    ...
    return (
        <div className="relative" ref={wrapperRef}>
            ...
            >
                <span className={cn("text-sm", selectedItem === 'all' ? "text-gray-500" : "text-gray-900 font-medium")}>
                    {selectedItem === 'all' ? placeholder : selectedItem}
                </span>
            ...
            {isOpen && (
                <div className="absolute top-full mt-1 w-[240px] bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-50 animate-in fade-in zoom-in-95 duration-100">
                    ...
                    <div className="max-h-[200px] overflow-y-auto">
                        ...
                        >
                            <span className={selectedItem === 'all' ? "text-blue-600 font-medium" : "text-gray-700"}>
                                {label}
                            </span>
                            {selectedItem === 'all' && <Check className="w-3.5 h-3.5 text-blue-600" />}
                        </button>
                        ...
                        {filteredItems.map(item => (
                            <button
                                ...
                                className="w-full px-4 py-2 text-left text-sm hover:bg-gray-50 flex items-center justify-between group"
                            >
                                <span className={selectedItem === item ? "text-blue-600 font-medium" : "text-gray-700"}>
                                    {item}
                                </span>
                                {selectedItem === item && <Check className="w-3.5 h-3.5 text-blue-600" />}
                            </button>
                        ))}
